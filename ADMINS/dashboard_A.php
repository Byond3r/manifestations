...
<body>
<a href="../ADMINS/connexion/déconnexion.php" class="logout-btn">
...
<!-- Menu d'actions rapides -->
    <div class="quick-actions">
        ...
        <a href="../Tableau_de_bord/vue_activité/choix_activité.php" class="quick-action-btn">
            <i class="fas fa-tasks"></i> Choix des activités
        </a>
        <a href="../ADMINS/gestion_comptes/gestion_comptes.php" class="quick-action-btn">
    ...
    <h3>Liste des Responsables et Participants par Activité</h3>

    <!-- Affichage des activités et leurs responsables -->
    <?php foreach ($activities as $activity) { 
        ...
        // Récupérer les responsables de l'activité
        $stmt = $conn->prepare("
            SELECT compte.Nom_C, compte.Prenom_C 
            FROM compte 
            INNER JOIN responsable ON compte.id_U = responsable.id_U 
            WHERE responsable.id_C = ?
        ");
        $stmt->execute([$activity['id_C']]);
            ...
            <?php if (count($responsables) > 0) { ?>
                <ul>
                    <?php foreach ($responsables as $responsable) { ?>
                        <li><i class="fas fa-user-tie"></i> <?= htmlspecialchars($responsable['Prenom_C']) . " " . htmlspecialchars($responsable['Nom_C']) ?></li>
                    <?php } ?>
                </ul>
            <?php } else { ?>
                <p class="no-participants"><i class="fas fa-info-circle"></i> Aucun responsable pour cette activité.</p>
            <?php } ?>
